...
def addGenericOptions(argParser):
    ...
    # NXYZ: Added arg
    argParser.add_argument("-o", "--output-path", dest="outputPath", help="") 
...
def writeRouteConf(duarouterBinary, step, options, dua_args, file,
    ...
    # NXYZ: Prepend output path instead:
    _config_out_file = os.path.join(options.outputPath, cfgname)
    _config_out_dir = os.path.dirname(_config_out_file)
    if not os.path.exists(_config_out_dir):
        os.makedirs(_config_out_dir, exist_ok=True)
    
    args += ["--save-configuration", _config_out_file] 
    ...
    # return cfgname
    # NXYZ: Prepend output path instead:
    return os.path.join(options.outputPath, cfgname)
...
def writeSUMOConf(sumoBinary, step, options, additional_args, route_files):
    ...
    sumoCmd = [sumoBinary,
            ...
            # NXYZ: Prepend output path instead:
               '--save-configuration', os.path.join(
                   options.outputPath,
                   "%s/iteration_%03i.sumocfg" % (step, step)
                ),
    ...
    # write detectorfile
    _detector_output_file = os.path.join(
        options.outputPath,
        str(step),
        detectorfile,
    )
    print("Detector output file: ", _detector_output_file)
    
    with open(_detector_output_file, 'w') as fd:
...
def main(args=None):
    ...
    for step in range(options.firstStep, options.lastStep):
        ...
        # NXYZ: Prepend output directory instead:
